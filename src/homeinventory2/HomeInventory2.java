...
public class HomeInventory2 extends JFrame
{
// Toolbar
...
static InventoryItem2[] myInventory = new InventoryItem2[maximumEntries];
int currentEntry;
...
{
// create frame
new HomeInventory2().show();
}
public HomeInventory2()
{
// frame constructor
...
{
myInventory[i] = new InventoryItem2();
...
//Suven Internship
myInventory[i - 2] = new InventoryItem2();
}
}
myInventory[currentEntry - 1] = new InventoryItem2();
...
{
myInventory[i - 1] = new InventoryItem2();
...
ImageIcon(HomeInventory2.photoTextArea.getText()).getImage();
int w = getWidth();
int h = getHeight();
...
{
Graphics2D g2D = (Graphics2D) g;
if ((pageIndex + 1) > HomeInventory2.lastPage)
{
return NO_SUCH_PAGE;
}
int i, iEnd;
...
iEnd = HomeInventory2.entriesPerPage * (pageIndex + 1);
if (iEnd > HomeInventory2.numberEntries)
iEnd = HomeInventory2.numberEntries;
for (i = 0 + HomeInventory2.entriesPerPage * pageIndex; i < iEnd; i++)
{
// dividing line
...
g2D.drawString(HomeInventory2.myInventory[i].description, (int) pf.getImageableX(), y);
y += dy;
g2D.setFont(new Font("Arial", Font.PLAIN, 12));
g2D.drawString("Location: " + HomeInventory2.myInventory[i].location, (int)
(pf.getImageableX() + 25), y);
y += dy;
if (HomeInventory2.myInventory[i].marked)
...
HomeInventory2.myInventory[i].serialNumber, (int) (pf.getImageableX() + 25), y);
y += dy;
g2D.drawString("Price: $" + HomeInventory2.myInventory[i].purchasePrice + "Purchased on: " + HomeInventory2.myInventory[i].purchaseDate, (int) (pf.getImageableX() +25), y);
y += dy;
g2D.drawString("Purchased at: " +HomeInventory2.myInventory[i].purchaseLocation, (int) (pf.getImageableX() + 25), y);
y += dy;
g2D.drawString("Note: " + HomeInventory2.myInventory[i].note, (int)
(pf.getImageableX() + 25), y);
y += dy;
try
{
...
ImageIcon(HomeInventory2.myInventory[i].photoFile).getImage();
